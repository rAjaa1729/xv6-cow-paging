...
void inituvm(pde_t *pgdir, char *init, uint sz)
{
  ...
  // cprintf("In inituvm increasing page dir %d %d\n", *pgdir, pte);
...
int allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for (; a < newsz; a += PGSIZE)
  {
    ...
    // cprintf("In alloc Old size %d %d new size %d %d %d %d\n",*pgdir,a,oldsz,newsz,pte,V2P(mem)/PGSIZE);
    inc_sharing(pte, V2P(mem) / PGSIZE);
    myproc()->rss += PGSIZE;
  }
  return newsz;
...
copyuvm(pde_t *pgdir, uint sz,struct proc* child)
{
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    *new_pte = *pte;
    ...
    // inc_sharing(new_pte,V2P(mem)/PGSIZE);
